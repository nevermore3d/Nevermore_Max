...
**SEALING**
*2mm* neoprene/silicone/rubber *sponge cord* (~3meters). The soft and squishy kind, Shore 00 range hardness (anything around 50 +-15 will work well). About
*3mm* neoprene/silicone/rubber sponge cord (~1meter).

**BOLTS**
M3 x 40 SHCS [4x]
M3 x 75 SHCS [4x]
ULTRA THIN HEAD M3 x 6mm (CBSKE-6) or 8mm (CBSKE-8) [~20x]. Doesnt have to be CBSKE bolts per se, but lets define ultra thin head as a head thickness at <=0.8mm. For reference, a m3 shcs bolt head is 3mm and a bhcs bolt head is 1.6mm.
M5 x 10 BHCS [4x]

A few m3s in regular sizes (6-20mm, exact count coming up).
...
* This was developed with and for **ABS+** material. Regular ABS *may* work depending on your printer, chamber heat and settings, but can also create *alot of unneccesary woes*! Printing a full bed size piece **will cause warping** in warp prone materials - hence do consider ABS+. I print in titanX - an ABS+ material - on PEI with no issues. Since this will be positioned outside the printer, there is also the option to print in other matierals. While I wouldn't print this in PLA if you plan on filtering a hot chamber that might reach above 60-70C under certain circumstances, PETG is probably fine. If moving outside of ABS+, I would personally opt for *carbon fiber* infused options when available (i.e CF-PETG, CF-Nylon, CF-ABS) as the long thin walls of the shell have a tendency to bow from light resistance from the sealing cord - putting the seal integrity in jeopardy - so a more rigid material choice is probably wise.
	...
	* **Parts shrinkage** affect the size of the transparent front panel! Any DXF available on the repo will be adjusted for ABS+ shrinkage (about 0.5%, meaning a 298mm part will actually come out being only 296,5mm). Regular ABS filament can often have more shrinkage, PLA doesnt shrink at all, and PETG might only shrink 0.25% - this affect the panel size! Do print a 300mm test piece and measure it to see your shrinkage factor before ordering a front panel, if you are not using A BS+ that is.
